...
[完整任务优先采用最优方案]
- Date: 2026-08-27
- Context: 用户要求整体重构 Android 本地 Agent、LSPosed 提权链和页面布局时明确指定
- Category: 工作流协作
- Instructions:
  - 面对完整任务时自行完成技术选型和端到端实施，以长期可维护、功能真实可用、可验证交付为优先目标。
  - 不为节省工作量而默认选择最小改动；当局部修补无法满足目标时，应主动实施必要的架构重构。
  - 执行过程中持续推进分析、实现、测试、构建和发布闭环，不因常规决策点反复暂停等待确认。
