// 云端任务聊天附件上传管线(约束对齐 web frontend 的 task-file-upload.tsx):
// 单条消息最多 3 个附件、单文件 2MB;超过 200KB 的可压缩图片先经 canvas 转
// webp(质量 0.6)缩体积,压不动(变大/编码失败)回退原图再按 2MB 拦截。
// 实际上传经壳命令 mc_upload(presign + PUT 都在壳内完成,凭证不出内核,
// 也避开 WebView 直传对象存储的跨域限制)。
import { mcUpload } from "./cloudapi";
import type { CloudAttachment } from "./types";
...
export interface CloudUploadedAtt extends CloudAttachment {
  isImage: boolean;
  preview?: string;
}
...
/** File → dataURL(聊天附件与文件抽屉上传共用;base64 取逗号后段) */
export function readDataURL(f: File): Promise<string> {
...
export async function uploadCloudFile(f: File): Promise<CloudUploadedAtt> {
  ...
  if (file.size > MAX_CLOUD_FILE_BYTES) throw new Error(`${name} 压缩后仍超过 2MB 上限`);
  ...
  return { url: access_url, filename: name, isImage, preview: isImage ? dataURL : undefined };
}
